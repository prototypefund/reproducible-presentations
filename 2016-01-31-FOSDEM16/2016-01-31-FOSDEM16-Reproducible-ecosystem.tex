...
\begin{frame}
 ...
 \begin{itemize}
  ...
  \item \only<1>{DebConf organizer}\only<2-3>{\sout{DebConf organizer}},
  founded the DebConf video team (\texttt{http://video.debian.net})
  \item \only<1>{Debian-Edu}\only<2-3>{\sout{Debian-Edu}} (Debian for education)
  ...
  \end{itemize}
  \item \only<1>{Debian LTS}\only<2-3>{\sout{Debian LTS}} (Long Term Support)
  \item<3> \texttt{sudo apt-get install torbrowser-launcher}
 \end{itemize}
\end{frame}
