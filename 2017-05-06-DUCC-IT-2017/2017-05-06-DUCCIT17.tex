...
\begin{frame}
 \frametitle{about Mattia}

 \begin{itemize}
  \item \small{\texttt{66AE 2B4A FCCF 3F52 DA18  4D18 4B04 3FCD B944 4540}}
  \item Ubuntu/Debian contributor since ~2010
  \item Ubuntu Developer since the last December
  \item Debian Developer since the second last December
  \begin{itemize}
    ...
    \begin{itemize}
      \item since August 2016 funded by the Linux Foundation
    \end{itemize}
    \item Quite a bunch of other stuff \ldots
  \end{itemize}
  \end{itemize}
\end{frame}
 ...
 \begin{center}
  \begin{columns}
   ...
   \column{.30\linewidth}
    ...
    Maria Glukhova \\
    Mathieu Bridon \\
    \only<1>{Mattia Rizzolo}\only<2>{{\color{debianred} Mattia Rizzolo}} \\
    Nicolas Boulenguez \\
    {Niels Thykier} \\
    \column{.30\linewidth}
    Niko Tyni \\
...
\begin{frame}
 ...
 \begin{itemize}
  \item these slides contain 5 week old data.
  \item …
  \end{itemize}
\end{frame}
...
\begin{frame}
 \frametitle{Progress in Debian \texttt{unstable} 5 weeks later}
 \begin{tikzpicture}[remember picture]
  \node[shift={(-0.5\paperwidth, \paperheight)},at=(current page.south east)] {
    \includegraphics[height=0.65\paperheight]{images/stats_pkg_state_unstable_5_weeks_later.png}
  };
 \end{tikzpicture}
 \begin{center}
  \footnotesize{22,950 (87.6\%) out of 26,189 source packages are reproducible \\
    in our test framework on \texttt{amd64}} (many build path variations resolved)
  \vfill
 \end{center}
\end{frame}
...
\begin{frame}
 ...
 \begin{itemize}
  \item \url{https://tests.reproducible-builds.org/$src}
...
\begin{frame}
 ...
 \begin{itemize}
  ...
  \item Canonical can take our work now and make Ubuntu 17.10
...
\begin{frame}
 ...
 \begin{itemize}
  \item "We don't care about Debian (only), we care about free and open
   source software."
  \item<2> 105 Weekly reports since May 2015
   \begin{itemize}
...
\begin{frame}
 ...
 \begin{itemize}
  \item<1-5> Stop using build dates
  \item<2-5> Stop using build dates
  \item<3-5> Do you really need build dates?
  \item<4-5> Seriously?
  \item<5> Then use \texttt{SOURCE\_DATE\_EPOCH} instead
  \item<5> See \url{https://reproducible-builds.org/specs/source-date-epoch/}
 \end{itemize}
\end{frame}
...
\begin{frame}
 ...
 \begin{itemize}
  \item
    ...
    \item \textbf{Dario Cavedon} for doing the by far greatest part of
        organization for the DUCC-IT
...
\end{itemize}
 ...
 \begin{center}
  \resizebox{0.9\textwidth}{!}{%
   \begin{tabular}{rl}
    \texttt{mattia@debian.org} & \texttt{66AE 2B4A FCCF 3F52 DA18} \\
                               & \texttt{4D18 4B04 3FCD B944 4540}
\end{tabular}
  }
 \end{center}
\end{frame}
...
\begin{textblock}{12}(2, 6)
    \tiny{
      Copyright \copyright{} 2014--2017 \\
         Holger Levsen <\texttt{holger@layer-acht.org}>
         Mattia Rizzolo <\texttt{mattia@debian.org}>
         and many others.\\[3.0mm]
      ...
      \url{https://gitlab.com/mattia/talks} and
